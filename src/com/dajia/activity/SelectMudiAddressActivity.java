...
//选择下单目的地地图
public class SelectMudiAddressActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		select_start=super.getIntent().getIntExtra("select_start", 0);
		select_end=super.getIntent().getIntExtra("select_end", 0);
		...
		Log.d(TAG, "screen metrics width:" + mWindowWidth + " heitht:"
				+ mWindowHeight + " densityDpi/160:" + mDpi);
		mCenter = new Rect(mWindowWidth / 2 - 30 * mDpi, mWindowHeight / 2 - 68
				* mDpi, mWindowWidth / 2 + 30 * mDpi, mWindowHeight / 2 - 8
				* mDpi);
	...
	private void addAddress() {
		if (mHasAddress) {
			...
			editor.putString("mudiaddress", mapAddress.getText().toString());
			editor.commit();*/
			...
			String address=(String) mapAddress.getText();
	...
	public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
		...
		//如果是出发地，将其经纬度保存下来
		if(select_start==99){
			  latS=(double) result.getLocation().latitude;
			  lonS=(double) result.getLocation().longitude;
			  editor.putString("x", lonS+"");
			  editor.putString("y", latS+"");
		}
		//如果是目的地，将其经纬度保存下来
		if(select_end==88){
			 latE=(double) result.getLocation().latitude;
			  lonE=(double) result.getLocation().longitude;
			  editor.putString("x2", lonE+"");
			  editor.putString("y2", latE+"");
		}
		editor.commit();
	}
