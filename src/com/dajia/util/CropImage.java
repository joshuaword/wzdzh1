...
public class CropImage{
	...
	public CropImage(Context context, CropImageView imageView,Handler handler){
		...
		mHandler = handler;
		//初始化图片保存路径
		FILE_LOCAL = new File(FileUtil.getImageDownFullDir());
	...
	public class BackgroundJob implements Runnable{
	    ...
	    public void run(){
    		...
    		mHandler.post(new Runnable() {
                public void run() {
                    try{
                    	mHandler.sendMessage(mHandler.obtainMessage(Constant.SHOW_PROGRESS));
                    }catch (Exception e) {
					}
                    latch.countDown();
                }
    		...
    		try {
    			...
    			mHandler.sendMessage(mHandler.obtainMessage(Constant.REMOVE_PROGRESS));
    		}
    	}
    }
}
