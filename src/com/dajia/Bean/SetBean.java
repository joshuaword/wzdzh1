package com.dajia.Bean;

import java.util.List;

import android.util.Log;

public class SetBean {
	private String ret;
	private String kefudianhua;
	private String appmap;//地图缩放比例
	

	private String ifhavedingdan;
	private String ifshowfuwuxiangmu;
	private String fenxiang;
	private String fenxiangurl;
	private String weizhangchaxun ;
	private List<FuwuxiangmuBean> fuwuxiangmu, fuwushijian,caidanguanggao;

	private String quxiao;
	private String quxiaoyuanyin1;
	private String quxiaoyuanyin2;
	private String quxiaoyuanyin3;
	private String ifyanzhenma;
	private String ifshowmudidi, ifxiadancongzi,ifnodrivernodan;
	private String ifshowfuwushijian, fuwushijian1, fuwuxiangmu1;
	private String baseurl, weiweb,shangwu;
	private String appbanquan;
	private String ifshowdrivernum;
	private String ifcanyuyue;//是否可预约
	private String yuyueneedtime;
	private String yuyuebegintime;
	private String yuyueendtime;
	private String  voice;
	private String clientusername,androidstore,wentifankui,gerenzhongxin,gongnengjieshao;
	
	
	public String getAndroidstore() {
		return androidstore;
	}

	public void setAndroidstore(String androidstore) {
		this.androidstore = androidstore;
	}

	public String getWentifankui() {
		return wentifankui;
	}

	public void setWentifankui(String wentifankui) {
		this.wentifankui = wentifankui;
	}

	public String getGerenzhongxin() {
		return gerenzhongxin;
	}

	public void setGerenzhongxin(String gerenzhongxin) {
		this.gerenzhongxin = gerenzhongxin;
	}

	public String getGongnengjieshao() {
		return gongnengjieshao;
	}

	public void setGongnengjieshao(String gongnengjieshao) {
		this.gongnengjieshao = gongnengjieshao;
	}

	public String getClientusername() {
		return clientusername;
	}

	public void setClientusername(String clientusername) {
		this.clientusername = clientusername;
	}

	public String getVoice() {
		return voice;
	}

	public void setVoice(String voice) {
		this.voice = voice;
	}

	public String getYuyueneedtime() {
		return yuyueneedtime;
	}

	public void setYuyueneedtime(String yuyueneedtime) {
		this.yuyueneedtime = yuyueneedtime;
	}

	public List<FuwuxiangmuBean> getCaidanguanggao() {
		return caidanguanggao;
	}

	public void setCaidanguanggao(List<FuwuxiangmuBean> caidanguanggao) {
		this.caidanguanggao = caidanguanggao;
	}

	

	public String getYuyuebegintime() {
		return yuyuebegintime;
	}

	public void setYuyuebegintime(String yuyuebegintime) {
		this.yuyuebegintime = yuyuebegintime;
	}

	public String getYuyueendtime() {
		return yuyueendtime;
	}

	public void setYuyueendtime(String yuyueendtime) {
		this.yuyueendtime = yuyueendtime;
	}

	public String getIfcanyuyue() {
		return ifcanyuyue;
	}

	public void setIfcanyuyue(String ifcanyuyue) {
		this.ifcanyuyue = ifcanyuyue;
	}

	public String getAppmap() {
		return appmap;
	}

	public void setAppmap(String appmap) {
		this.appmap = appmap;
	}
	public String getIfshowdrivernum() {
		return ifshowdrivernum;
	}

	public void setIfshowdrivernum(String ifshowdrivernum) {
		this.ifshowdrivernum = ifshowdrivernum;
	}

	public String getAppbanquan() {
		return appbanquan;
	}

	public void setAppbanquan(String appbanquan) {
		this.appbanquan = appbanquan;
	}

	public String getShangwu() {
		return shangwu;
	}

	public void setShangwu(String shangwu) {
		this.shangwu = shangwu;
	}

	private String ifshowcheliang;
	
	public String getIfshowcheliang() {
		return ifshowcheliang;
	}

	public void setIfshowcheliang(String ifshowcheliang) {
		this.ifshowcheliang = ifshowcheliang;
	}

	public String getWeiweb() {
		return weiweb;
	}

	public void setWeiweb(String weiweb) {
		this.weiweb = weiweb;
	}
	public String getIfnodrivernodan() {
		return ifnodrivernodan;
	}

	public void setIfnodrivernodan(String ifnodrivernodan) {
		this.ifnodrivernodan = ifnodrivernodan;
	}
	public String getWeizhangchaxun() {
		return weizhangchaxun;
	}

	public void setWeizhangchaxun(String weizhangchaxun) {
		this.weizhangchaxun = weizhangchaxun;
	}

	public String getBaseurl() {
		return baseurl;
	}

	public void setBaseurl(String baseurl) {
		this.baseurl = baseurl;
	}
	
	public String getKefudianhua() {
		return kefudianhua;
	}

	public void setKefudianhua(String kefudianhua) {
		this.kefudianhua = kefudianhua;
	}

	

	public String getIfhavedingdan() {
		return ifhavedingdan;
	}

	public void setIfhavedingdan(String ifhavedingdan) {
		this.ifhavedingdan = ifhavedingdan;
	}

	public String getRet() {
		return ret;
	}

	public void setRet(String ret) {
		this.ret = ret;
	}

	public String getIfshowfuwuxiangmu() {
		return ifshowfuwuxiangmu;
	}

	public void setIfshowfuwuxiangmu(String ifshowfuwuxiangmu) {
		this.ifshowfuwuxiangmu = ifshowfuwuxiangmu;
	}

	public String getFenxiang() {
		return fenxiang;
	}

	public void setFenxiang(String fenxiang) {
		this.fenxiang = fenxiang;
	}

	public String getFenxiangurl() {
		return fenxiangurl;
	}

	public void setFenxiangurl(String fenxiangurl) {
		this.fenxiangurl = fenxiangurl;
	}

	public List<FuwuxiangmuBean> getFuwuxiangmu() {
		return fuwuxiangmu;
	}

	public void setFuwuxiangmu(List<FuwuxiangmuBean> fuwuxiangmu) {
		this.fuwuxiangmu = fuwuxiangmu;
	}

	public List<FuwuxiangmuBean> getFuwushijian() {
		return fuwushijian;
	}

	public void setFuwushijian(List<FuwuxiangmuBean> fuwushijian) {
		this.fuwushijian = fuwushijian;
	}
	
	public String getQuxiao() {
		return quxiao;
	}

	public void setQuxiao(String quxiao) {
		this.quxiao = quxiao;
	}

	public String getQuxiaoyuanyin1() {
		return quxiaoyuanyin1;
	}

	public void setQuxiaoyuanyin1(String quxiaoyuanyin1) {
		this.quxiaoyuanyin1 = quxiaoyuanyin1;
	}

	public String getQuxiaoyuanyin2() {
		return quxiaoyuanyin2;
	}

	public void setQuxiaoyuanyin2(String quxiaoyuanyin2) {
		this.quxiaoyuanyin2 = quxiaoyuanyin2;
	}

	public String getQuxiaoyuanyin3() {
		return quxiaoyuanyin3;
	}

	public void setQuxiaoyuanyin3(String quxiaoyuanyin3) {
		this.quxiaoyuanyin3 = quxiaoyuanyin3;
	}

	public String getIfyanzhenma() {
		return ifyanzhenma;
	}

	public void setIfyanzhenma(String ifyanzhenma) {
		this.ifyanzhenma = ifyanzhenma;
	}

	public String getIfxiadancongzi() {
		return ifxiadancongzi;
	}

	public void setIfxiadancongzi(String ifxiadancongzi) {
		this.ifxiadancongzi = ifxiadancongzi;
	}
	
	
	public String getIfshowmudidi() {
		return ifshowmudidi;
	}

	public void setIfshowmudidi(String ifshowmudidi) {
		this.ifshowmudidi = ifshowmudidi;
	}
	
	public String getFuwuxiangmu1() {
		return fuwuxiangmu1;
	}

	public void setFuwuxiangmu1(String fuwuxiangmu1) {
		this.fuwuxiangmu1 = fuwuxiangmu1;
		Log.e("MUSIC", "setbean fuwuxiangmu1="+fuwuxiangmu1);
	}
	
	public String getFuwushijian1() {
		return fuwushijian1;
	}

	public void setFuwushijian1(String fuwushijian1) {
		this.fuwushijian1 = fuwushijian1;
	}

	public String getIfshowfuwushijian() {
		return ifshowfuwushijian;
	}

	public void setIfshowfuwushijian(String ifshowfuwushijian) {
		this.ifshowfuwushijian = ifshowfuwushijian;
	}

	
	
	
}
