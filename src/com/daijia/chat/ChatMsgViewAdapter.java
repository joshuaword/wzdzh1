...
public class ChatMsgViewAdapter extends BaseAdapter {
	...
	public static void setCacheImageLoad(Context context, int drawId, int Rounded, ImageView imageView,
		...
		options = new DisplayImageOptions.Builder().showStubImage(drawId).showImageForEmptyUri(drawId)
				.showImageOnFail(drawId).cacheInMemory().cacheOnDisc().displayer(new RoundedBitmapDisplayer(Rounded))
				.build();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			viewHolder.img_chatcontent = (ImageView) convertView.findViewById(R.id.img_chatcontent);
			viewHolder.img_play = (ImageView) convertView.findViewById(R.id.img_play);
			viewHolder.url_layout = convertView.findViewById(R.id.url_layout);
		...
		} else if (!entity.getThefile().equals("") && entity.getLeixing().equals("video")) {
			viewHolder.img_chatcontent.setVisibility(View.VISIBLE);
			viewHolder.img_play.setVisibility(View.VISIBLE);
			viewHolder.tvContent.setVisibility(View.GONE);
			viewHolder.url_layout.setVisibility(View.GONE);
			Log.d("entity.getThefile()", entity.getThefile());
			setCacheImageLoad(ctx, 0, 0, viewHolder.img_chatcontent, entity.getIcon());
		} else {
			viewHolder.img_chatcontent.setVisibility(View.GONE);
			viewHolder.img_play.setVisibility(View.GONE);
			viewHolder.img_chatcontent.setVisibility(View.GONE);
		...
		viewHolder.img_chatcontent.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (entity.getLeixing().equals("img")) {
					imglist.clear();
					imglist.add(entity.getThefile());
					Intent intent = new Intent(ctx, ImageGesActivity.class);
					intent.putExtra("selectposition", 0);
					intent.putStringArrayListExtra("imglist", imglist);
					ctx.startActivity(intent);
				} else {
					Intent intent = new Intent(ctx, VideoTasteActivity.class);
					intent.putExtra("url", entity.getThefile());
					ctx.startActivity(intent);
				}
